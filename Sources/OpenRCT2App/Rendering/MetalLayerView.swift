...
import QuartzCore
    ...
    final class MetalLayerView: UIView {
        ...
        private func ensureEngineInitialized() -> Bool {
            ...
            // Step 5: Use Bundle.main.url() to get actual bundle resource paths
            // This ensures we're reading from the correct location in the visionOS sandbox
            var actualBundleResourcePath = bundleRoot

            if let g2URL = Bundle.main.url(forResource: "g2", withExtension: "dat") {
                // Get the directory containing g2.dat - that's our actual resource base path
                actualBundleResourcePath = g2URL.deletingLastPathComponent().path
                log("[Init] Bundle.main.url found g2.dat at: \(g2URL.path)")
                log("[Init] Using actual bundle resource path: \(actualBundleResourcePath)")
            } else {
                log(
                    "[Init] Bundle.main.url could NOT find g2.dat - using bundleRoot: \(bundleRoot)"
                )
            }

            let resourceBasePath = actualBundleResourcePath
            let documentsPath =
            ...
            let fileExists: (String) -> Bool = { path in
                ...
                if !exists {
                    self.log("[Init] File MISSING: \(path)")
                }
                return exists
            }
            ...
            let g2Path = resourceBasePath + "/g2.dat"
            let fontsPath = resourceBasePath + "/fonts.dat"
            let tracksPath = resourceBasePath + "/tracks.dat"
            let languagePath = resourceBasePath + "/language"
            let g1Path = resourceBasePath + "/rct2/Data/g1.dat"

            log("[Init] bundleRoot=\(bundleRoot)")
            log("[Init] actualResourcePath=\(resourceBasePath)")
            log(
                "[Init] resources g2=\(fileExists(g2Path)) fonts=\(fileExists(fontsPath)) tracks=\(fileExists(tracksPath)) langDir=\(dirExists(languagePath))"
            )
            ...
            resourceBasePath.withCString { bundleCStr in
                documentsPath.withCString { userCStr in
                    cachePath.withCString { cacheCStr in
                        log(
                            "[Init] Calling openrct2_set_paths with bundlePath: \(resourceBasePath)"
                        )
            ...
            if !openrct2_init_full() {
                let error = openrct2_get_init_error().map { String(cString: $0) } ?? "Unknown error"
                emitError(
                    "openrct2_init_full failed: \(error). Check OpenRCT2 resources at \(resourceBasePath)."
                )
            } else {
