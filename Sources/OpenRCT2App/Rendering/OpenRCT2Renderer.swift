...
final class OpenRCT2Renderer {
    ...
    init() throws {
        ...
        try initializeDrawableQueue(
            width: OpenRCT2Renderer.DEFAULT_WIDTH, height: OpenRCT2Renderer.DEFAULT_HEIGHT)
    ...
    private func initializeBuffers() throws {
        ...
        guard
            let indexedBuf = device.makeBuffer(length: pixelBufferSize, options: .storageModeShared)
        else {
        ...
        guard let paletteBuf = device.makeBuffer(length: paletteSize, options: .storageModeShared)
        else {
            throw RendererError.bufferCreationFailed
        }
